...
list_agree <-
  merge(x = list_agree_P, y = list_agree_LC, by = "organization_id") %>% #分別讀取人事室及教務處審核同意名單後合併
  subset(agree.x == 1 & agree.y == 1) %>% #只留下人事室及教務處皆審核同意的名單
  mutate(agree = 1) %>%
  select(organization_id, agree)
	...
	where sheet_id = (SELECT [id] FROM [plat5_edhr].[dbo].[row_sheets]
          ...
          if (if (!(dim(teacher)[1] == 0 |
            ...
            (
              ...
              identical(
                as.data.frame(data_load_check),
                as.data.frame(data_load_check_pre)
              )
              ...
              (
                ...
                all(
                  data_load_check_count$count == data_load_check_count$count_pre
                ) &
